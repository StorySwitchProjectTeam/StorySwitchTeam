...
<head>
    ...
    <title>StorySwitch Editor V11 (Final)</title>
    ...
    <style>
        ...
        /* Drag Overlay pour la médiathèque */
...
<body>
    ...
    <div id="fatal-error-overlay">
        <div class="bg-red-900 p-8 rounded-lg max-w-lg border border-red-500">
            ...
            <p class="mb-6 text-gray-300">Passe par Laragon : http://localhost/ton_dossier/creation.php</p>
            <button onclick="location.reload()" class="bg-white text-red-900 font-bold py-2 px-6 rounded hover:bg-gray-200">Réessayer</button>
        </div>
    </div>
    ...
    <header class="toolbar">
        ...
        <div class="flex items-center gap-3">
            ...
            <button id="btn-videos" class="btn btn-dark relative"><i class="fa-regular fa-folder-open"></i> Médiathèque <span id="badge-videos" class="absolute -top-2 -right-2 bg-blue-600 text-[10px] px-1.5 rounded-full hidden">0</span></button>
        ...
        <div class="modal-box w-[800px] flex flex-col bg-[#18181b] max-h-[90vh]">
            ...
            <div class="p-4 bg-[#27272a] border-b border-[#3f3f46]">
                ...
                <div class="flex gap-2">
                    ...
                    <button id="btn-delete-server" class="btn btn-danger" title="Supprimer ce fichier"><i class="fa-solid fa-trash"></i></button>
                </div>
            </div>
    ...
    <div id="modal-library" class="modal-overlay">
        <div class="modal-box w-[600px] flex flex-col bg-[#18181b] max-h-[80vh]">
            <div class="flex justify-between p-4 border-b border-[#3f3f46]">
                <h3 class="font-bold text-white"><i class="fa-solid fa-film mr-2"></i> Médiathèque</h3>
                <button class="text-gray-400 hover:text-white close-modal"><i class="fa-solid fa-times"></i></button>
            </div>
            
            <div id="lib-dropzone" class="flex-1 p-4 overflow-y-auto bg-[#09090b] relative transition-all duration-200 border-2 border-transparent">
                <div id="drop-hint" class="hidden absolute inset-0 bg-blue-500/10 flex flex-col items-center justify-center z-50 pointer-events-none backdrop-blur-sm">
                    <i class="fa-solid fa-cloud-arrow-up text-4xl text-blue-400 mb-2"></i>
                    <span class="text-blue-200 font-bold">Lâchez pour uploader !</span>
                </div>

                <div id="video-list-container" class="flex flex-col gap-2 min-h-full">
                    </div>
            </div>

            <div class="p-4 border-t border-[#3f3f46] bg-[#27272a] flex justify-between items-center">
                <span class="text-xs text-gray-400" id="lib-stats">0 vidéos</span>
                <button id="btn-add-videos" class="btn btn-primary">
                    <i class="fa-solid fa-cloud-arrow-up"></i> Ajouter des vidéos
                </button>
                <input type="file" id="input-videos-hidden" multiple accept="video/*" class="hidden">
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        const PHP_DELETE_JSON = 'delete_json.php';
        const PHP_DELETE_VIDEO = 'delete_video.php';
        ...
        // --- INIT ---
        function init() {
            console.log("V11 Engine Started (Server Mode + Fixes + Library)");
            ...
            // Centrage correct : On décale le nœud pour qu'il soit au centre du 0,0
            const start = createNode(-140, -100, {name:'intro'});
            saveState("Init");
        }
        ...
        async function deleteServerFile() {
            const name = document.getElementById('server-file-select').value;
            if(!name) return toast("Aucun fichier sélectionné", "error");
            if(!confirm(`⚠️ Supprimer définitivement "${name}" ?`)) return;
            
            try {
                const res = await fetch(PHP_DELETE_JSON, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                ...
                if(data.success) {
                    ...
                    fetchJsonList(); document.getElementById('server-file-select').value = "";
        ...
        async function deleteVideo(filename) {
            ...
            try {
                const res = await fetch(PHP_DELETE_VIDEO, {
                    method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ filename: filename })
                });
                const data = await res.json();
                if(data.success) {
                    toast("Vidéo supprimée", "success");
                    App.videos = App.videos.filter(v => !v.endsWith(filename));
        ...
        function loadProject(d) {
            ...
            if(d.construct) { 
                ...
                App.view=d.construct.view || {x:0, y:0, z:1}; App.nextId=d.construct.nextId || 100; 
                Object.values(App.nodes).forEach(n => { if (isNaN(n.x) || isNaN(n.y)) { n.x=100; n.y=100; } });
            } else {
                let idx = 1; const nodeKeys = Object.keys(d.data_film || d);
                ...
                nodeKeys.forEach((k, i) => {
                    ...
                    App.nodes[id] = { id, name:k, x:pos[i].x, y:pos[i].y, url:nodeData.url||'', timer:nodeData.duree_choix||0, choices:[] };
                    if(nodeData.choix) Object.values(nodeData.choix).forEach(c => App.nodes[id].choices.push(c[0]));
                });
                nodeKeys.forEach((k) => {
                   const nodeData = (d.data_film && d.data_film[k]) ? d.data_film[k] : d[k];
                   if(nodeData.choix) Object.values(nodeData.choix).forEach(c => { if(c[1] && map[c[1]]) mkLink(map[k], `${c[0]}_out`, map[c[1]], 'in'); });
                });
                App.nextId = idx + 1; App.view = {x: innerWidth/2, y: innerHeight/2, z: 1};
            }
            ...
            setTimeout(() => {
                ...
                saveState("Load"); toast("Projet chargé", "success");
            }, 50);
        }
            ...
            el.innerHTML = `
                ...
                <div class="p-3 flex flex-col gap-2">
                    ...
                    <div class="flex items-center gap-2 bg-[#111] border border-[#333] rounded p-1">
                        ...
                        <input type="number" step="0.1" class="bg-transparent border-none text-gray-300 text-xs w-full outline-none tm-in" value="${n.timer / 1000}" placeholder="s">
                    </div>
        ...
        function bindNode(el, n) {
            ...
            el.querySelector('.tm-in').onchange = e => { n.timer=(parseFloat(e.target.value)*1000)||0; saveState("Time"); };
            ...
            el.querySelectorAll('.del-ch').forEach(b=>b.onclick=e=>{
                const idx=parseInt(e.target.dataset.idx); const val=n.choices[idx];
            ...
            el.querySelectorAll('.ch-in').forEach(i=>i.onchange=e=>{
                const idx=parseInt(e.target.dataset.idx); const old=n.choices[idx]; const val=e.target.value;
        ...
        function bindEvents() {
            ...
            window.addEventListener('keydown', e => {
                ...
                if(e.code==='Space') { App.keys['Space']=true; document.body.style.cursor='grab'; e.preventDefault(); }
            ...
            window.addEventListener('keyup', e => {
                if(e.code==='Space') { App.keys['Space']=false; document.body.style.cursor='default'; if(App.drag?.type==='pan') App.drag=null; }
            });
            ...
            window.addEventListener('mousedown', e => {
                ...
                if(e.button===1 || (e.button===0 && App.keys['Space'])) {
                ...
                if(e.button===0 && t===DOM.vp && !App.keys['Space']) {
            ...
            window.addEventListener('mouseup', e => {
                if(App.drag?.type==='pan') document.body.style.cursor=App.keys['Space']?'grab':'default';
        ...
        function setupUI() {
            ...
            // Médiathèque Logic (AVEC DRAG & DROP)
            const btnVid = document.getElementById('btn-videos');
            const modLib = document.getElementById('modal-library');
            const btnAddVid = document.getElementById('btn-add-videos');
            const hiddenInput = document.getElementById('input-videos-hidden');
            const dropZone = document.getElementById('lib-dropzone');
            const dropHint = document.getElementById('drop-hint');
            
            btnVid.onclick = () => { renderLibrary(); modLib.classList.add('active'); };
            btnAddVid.onclick = () => hiddenInput.click();
            hiddenInput.onchange = e => { if(e.target.files.length) uploadFiles(e.target.files); };

            // LOGIQUE DRAG & DROP
            dropZone.addEventListener('dragover', (e) => {
                e.preventDefault(); e.stopPropagation();
                dropZone.classList.add('drag-over'); dropHint.classList.remove('hidden');
            });
            dropZone.addEventListener('dragleave', (e) => {
                e.preventDefault(); e.stopPropagation();
                if (e.currentTarget.contains(e.relatedTarget)) return;
                dropZone.classList.remove('drag-over'); dropHint.classList.add('hidden');
            });
            ...
            document.getElementById('btn-save').onclick = () => { document.getElementById('save-filename').value = App.currentFile; document.getElementById('modal-save').classList.add('active'); };
            ...
            document.getElementById('act-export').onclick = () => { txt.value = JSON.stringify(buildDataFilm(), null, 2); mJson.classList.add('active'); fetchJsonList(); };
            document.getElementById('act-import').onclick = () => { txt.value=''; mJson.classList.add('active'); fetchJsonList(); };
            
            document.getElementById('btn-load-server').onclick = loadFromServer;
            document.getElementById('btn-delete-server').onclick = deleteServerFile;
            
            document.getElementById('btn-copy').onclick = () => { txt.select(); document.execCommand('copy'); toast("Copié !"); };
            document.getElementById('btn-import-conf').onclick = () => { try { loadProject(JSON.parse(txt.value)); mJson.classList.remove('active'); toast("Importé"); } catch(e) { alert("JSON Erreur: "+e.message); } };
            ...
            document.getElementById('act-clear').onclick = () => { 
                if(confirm("Créer un nouveau projet ?")) { 
                    ...
                    createNode(-140, -100, {name:'intro'}); App.currentFile = "nouveau_projet.json"; updatePlayerLink(null); saveState("New");
                } 
            };
        ...
        function renderLibrary() {
            ...
            container.innerHTML = ''; stats.innerText = `${App.videos.length} vidéos stockées`;
            if(App.videos.length === 0) { container.innerHTML = '<div class="text-gray-500 text-sm text-center italic py-4">Le dossier est vide.<br>Glissez des vidéos ici.</div>'; return; }
            App.videos.forEach(vid => {
                const name = vid.split('/').pop();
                const div = document.createElement('div');
                div.className = 'flex justify-between items-center bg-[#18181b] p-3 rounded border border-[#3f3f46] hover:border-blue-500 transition-colors group';
                div.innerHTML = `<div class="flex items-center gap-3 overflow-hidden"><div class="w-8 h-8 bg-black rounded flex items-center justify-center text-gray-500"><i class="fa-solid fa-video"></i></div><span class="text-sm text-gray-300 truncate font-mono select-all">${name}</span></div><button class="text-gray-500 hover:text-red-500 px-2 py-1 transition-colors delete-vid-btn" title="Supprimer"><i class="fa-solid fa-trash"></i></button>`;
                div.querySelector('.delete-vid-btn').onclick = () => deleteVideo(name);
        ...
        function updatePlayerLink(f) { if(f) { DOM.playerLink.href=`index.php?projet=${encodeURIComponent(f)}`; DOM.playerLink.style.opacity=1; DOM.playerLink.style.pointerEvents='auto'; } else { DOM.playerLink.removeAttribute('href'); DOM.playerLink.style.opacity=0.5; DOM.playerLink.style.pointerEvents='none'; } }
        function generateCircularPositions(count, centerX, centerY, radius) {
            const positions = []; const angleStep = (2 * Math.PI) / count;
            for (let i = 0; i < count; i++) { positions.push({ x: Math.round(centerX + radius * Math.cos(i * angleStep - Math.PI / 2)), y: Math.round(centerY + radius * Math.sin(i * angleStep - Math.PI / 2)) }); } return positions;
        }
        ...
        window.ctxAction = (act, p1, p2, p3, p4) => {
            ...
            if(act==='link-create') { const newNode = createNode(p1, p2); mkLink(p3, p4, newNode.id, 'in'); saveState("LinkCreate"); toast("Nouvelle scène reliée !"); }
            document.getElementById('context-menu').style.display='none';
        };
        ...
        function buildDataFilm() {
            ...
            Object.values(App.nodes).forEach(n=>{
                const ch={};
                n.choices.forEach((c,i)=>{ const l = Object.values(App.links).find(x=>x.srcNode===n.id && x.srcPort===`${c}_out`); ch[`choix${i+1}`] = [c, l?App.nodes[l.tgtNode].name:null]; });
        ...
        function drawMini() {
            ...
            ns.forEach(n=>{ ctx.fillStyle = App.selection.has(n.id)?'#3b82f6':'#52525b'; ctx.fillRect((n.x-minX+p)*s, (n.y-minY+p)*s, 280*s, 150*s); });
            const vx=(-App.view.x/App.view.z-minX+p)*s, vy=(-App.view.y/App.view.z-minY+p)*s;
            DOM.miniV.style.left=vx+'px'; DOM.miniV.style.top=vy+'px'; DOM.miniV.style.width=(innerWidth/App.view.z*s)+'px'; DOM.miniV.style.height=(innerHeight/App.view.z*s)+'px';
        }
        ...
        function loadHist(d) {
            ...
            if(ni>=0 && ni<App.history.length) {
                App.histIdx=ni; const o=JSON.parse(App.history[ni]); App.nodes=o.n; App.links=o.l; App.nextId=o.i;
                DOM.nLayer.innerHTML=''; DOM.sLayer.innerHTML=''; Object.values(App.nodes).forEach(n=>renderNode(n)); Object.values(App.links).forEach(l=>drawLink(l));
