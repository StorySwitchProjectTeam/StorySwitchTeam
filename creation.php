...
<head>
    ...
    <style>
        ...
        .cable:hover~defs #arrow-marker path {
            fill: var(--primary);
        }
...
<body>
    ...
    <div id="fatal-error-overlay">
        <div class="bg-red-900 p-8 rounded-lg max-w-lg border border-red-500">
            ...
            <p class="mb-6 text-gray-300">Le PHP ne peut pas fonctionner comme ça. Tu dois passer par ton serveur
                Laragon.</p>
            ...
            <button onclick="location.reload()"
                class="bg-white text-red-900 font-bold py-2 px-6 rounded hover:bg-gray-200">J'ai compris,
                réessayer</button>
        </div>
    </div>
    ...
    <div id="viewport">
        <div id="world">
            <svg id="connections-layer">
                <defs>
                    <marker id="arrow-marker" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto"
        ...
        <div id="selection-box"
            style="position: absolute; border: 1px solid #3b82f6; background: rgba(59,130,246,0.1); display: none; z-index: 999; pointer-events: none;">
        </div>
    </div>
    ...
    <header class="toolbar">
        <div class="flex items-center gap-4">
            <div
                class="w-8 h-8 bg-gradient-to-br from-blue-600 to-cyan-500 rounded-lg flex items-center justify-center shadow-lg">
            ...
            <div class="relative">
                ...
                <input type="text" id="search-input" placeholder="Filtrer..."
                    class="bg-black/20 border border-white/10 text-gray-300 text-sm rounded-full pl-9 pr-4 py-1.5 focus:border-blue-500 outline-none w-64 transition-all">
            </div>
        </div>
        <div class="flex items-center gap-3">
            <div class="flex bg-white/5 rounded-lg border border-white/10 p-0.5">
                <button id="btn-undo"
                    class="px-3 py-1.5 text-gray-400 hover:text-white hover:bg-white/5 rounded disabled:opacity-30"><i
            ...
            <button id="btn-videos" class="btn btn-dark relative"><i class="fa-regular fa-folder-open"></i> Vidéos <span
                    id="badge-videos"
                    class="absolute -top-2 -right-2 bg-blue-600 text-[10px] px-1.5 rounded-full hidden">0</span></button>
            ...
            <div class="relative">
                ...
                <div id="menu-data" class="dropdown-menu">
                    <div class="px-3 py-2 hover:bg-[#27272a] cursor-pointer rounded text-sm text-gray-300 flex items-center gap-2"
                        id="act-import"><i class="fa-solid fa-file-import text-green-500"></i> Importer / Charger</div>
                    <div class="px-3 py-2 hover:bg-[#27272a] cursor-pointer rounded text-sm text-gray-300 flex items-center gap-2"
                        id="act-export"><i class="fa-solid fa-file-export text-yellow-500"></i> Voir JSON Brut</div>
                    <div class="h-px bg-[#3f3f46] my-1"></div>
                    <div class="px-3 py-2 hover:bg-[#27272a] cursor-pointer rounded text-sm text-red-400 flex items-center gap-2"
                        id="act-clear"><i class="fa-solid fa-trash"></i> Nouveau Projet</div>
                </div>
            </div>
            ...
            <button id="btn-save" class="btn" style="background:#7c3aed; color:white;"><i
                    class="fa-solid fa-floppy-disk"></i> Sauvegarder</button>
            <a id="player-link" href="#" target="_blank" class="btn"
                style="background:#059669; color:white; pointer-events: none; opacity: 0.5;"><i
                    class="fa-solid fa-play"></i> Jouer</a>
        </div>
    </header>
    ...
    <div
        class="fixed bottom-0 left-0 right-0 bg-[#18181b] border-t border-[#333] px-6 py-1 flex justify-between items-center text-xs text-gray-500 z-50 pointer-events-auto">
        <div class="flex gap-4">
            <span><i class="fa-solid fa-mouse mr-1"></i> Zoom: <kbd>Molette</kbd></span>
            <span>Déplacement: <kbd>Clic Molette</kbd> ou <kbd>Espace+Drag</kbd></span>
        ...
        <div class="modal-box w-[800px] flex flex-col bg-[#18181b] max-h-[90vh]">
            ...
            <div class="p-4 bg-[#27272a] border-b border-[#3f3f46]">
                ...
                <div class="flex gap-2">
                    <select id="server-file-select"
                        class="flex-1 bg-[#18181b] border border-[#3f3f46] text-white rounded p-2 text-sm outline-none">
            ...
            <div class="flex-1 p-4 overflow-hidden flex flex-col">
                ...
                <textarea id="json-area"
                    class="w-full h-48 bg-[#09090b] text-green-400 font-mono text-xs p-4 rounded border border-[#3f3f46] outline-none resize-none mb-2"></textarea>
            </div>
    ...
    <div id="modal-save" class="modal-overlay">
        <div class="modal-box w-[400px] p-6 bg-[#18181b]">
            ...
            <input type="text" id="save-filename" value="projet.json"
                class="w-full bg-[#222] border border-[#444] text-white rounded p-2 mb-6 outline-none focus:border-purple-500"
                autofocus>
    ...
    <div id="modal-rename" class="modal-overlay">
        <div class="modal-box w-[350px] p-6 bg-[#18181b]">
            ...
            <input type="text" id="rename-input"
                class="w-full bg-[#222] border border-[#444] text-white rounded p-2 mb-6 outline-none focus:border-blue-500"
                autofocus>
            <div class="flex justify-end gap-2">
                <button class="btn btn-dark close-modal">Annuler</button>
    ...
    <div id="modal-settings" class="modal-overlay">
        <div class="modal-box w-[400px] bg-[#18181b]">
            <div class="p-4 border-b border-[#3f3f46] flex justify-between">
                <h3 class="font-bold text-white">Paramètres</h3><button class="close-modal text-gray-400"><i
                        class="fa-solid fa-times"></i></button>
            </div>
            <div class="p-6 flex flex-col gap-4">
                <div class="flex justify-between"><span class="text-sm text-gray-300">Minimap</span><input
                        type="checkbox" id="set-minimap" checked class="accent-blue-500"></div>
                <div class="flex justify-between"><span class="text-sm text-gray-300">Snap Grille</span><input
                        type="checkbox" id="set-snap" checked class="accent-blue-500"></div>
                <div class="flex justify-between"><span class="text-sm text-gray-300">Style Lignes</span><select
                        id="set-style" class="bg-[#222] text-xs rounded p-1">
                        <option value="bezier">Courbe</option>
    ...
    <script type="module">
        ...
        function bindEvents() {
            ...
            // --- GESTION CLAVIER (Appui) ---
            window.addEventListener('keydown', e => {
                // Si on est dans un champ texte, on laisse le clavier tranquille
                if(e.target.tagName.match(/INPUT|SELECT|TEXTAREA/)) return;

                // 1. SPACE : Pour le déplacement (Pan)
                if(e.code === 'Space') {
                    App.keys['Space'] = true; 
                    document.body.style.cursor = 'grab'; // Change le curseur en main
            ...
            // --- GESTION CLAVIER (Relâchement) ---
            window.addEventListener('keyup', e => {
                if(e.code === 'Space') {
                    App.keys['Space'] = false;
                    document.body.style.cursor = 'default'; // Remet le curseur normal
                    if(App.drag && App.drag.type === 'pan') App.drag = null; // Stoppe le drag si on lâche espace
                }
            });

            // --- CLIC SOURIS ---
            window.addEventListener('mousedown', e => {
                if (e.target.closest('#context-menu')) return; // Fix menu contextuel
                ...
                // Clic molette (button 1) OU (Clic gauche + Touche Espace maintenue)
                if(e.button===1 || (e.button===0 && App.keys['Space'])) {
                    App.drag = { type:'pan', sx:e.clientX, sy:e.clientY }; 
                    document.body.style.cursor='grabbing'; // Main fermée
                    return;
                }
                ...
                // Sélection Rectangle (Seulement si on ne tient pas Espace)
            ...
            window.addEventListener('mouseup', e => {
                if(App.drag?.type==='pan') {
                    // Si on lâche le clic mais qu'on tient encore Espace, on remet le curseur "grab" (main ouverte)
                    document.body.style.cursor = App.keys['Space'] ? 'grab' : 'default';
                }
            ...
            // Context Menu & Close logic (reste identique)
